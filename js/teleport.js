...
prepareToTeleport(direction){
         setTimeout(()=>{
    this.indexTeleportPair = board.elementContainer.teleportPair.indexOf(this.pairTeleport)
    if (this.indexTeleportPair != -1){
          this.nextTeleport = board.elementContainer.teleportName[this.indexTeleportPair]
 
   this.teleport(direction, eval(this.nextTeleport).startRowPosition, eval(this.nextTeleport).startColumnPosition)        
    } else {
         this.teleport(direction, this.startRowPosition, this.startColumnPosition)
    }
            }, 0)   
}
    ...
    if (this.checkMove) {
      ...
      if (
        ...
        board.robbo.startColumnPosition =
          teleportColumn + this.rooboColumn;
      } else {
